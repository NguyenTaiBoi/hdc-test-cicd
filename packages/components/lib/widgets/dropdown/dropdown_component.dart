...
class _DropdownMenuPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Tween<double> top = Tween<double>(
      begin:
          selectedItemOffset.clamp(0.0, math.max(size.height - itemHeight, 0)),
      end: 0,
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.itemIndex == widget.route.selectedIndex) {
      opacity = CurvedAnimation(
          parent: widget.route.animation!, curve: const Threshold(0));
    } else {
      final double start =
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    double maxHeight = math.max(0, constraints.maxHeight - 2 * itemHeight);
...
class DropdownButtonComponent<T> extends StatefulWidget {
  ...
  @override
  State<DropdownButtonComponent<T>> createState() =>
      _DropdownButtonComponentState<T>();
}

class _DropdownButtonComponentState<T> extends State<DropdownButtonComponent<T>>
  ...
  @override
  void didUpdateWidget(DropdownButtonComponent<T> oldWidget) {
...
class DropdownFormFieldComponent<T> extends FormField<T> {
              ...
              child: Builder(builder: (BuildContext context) {
                return InputDecorator(
                  ...
                  child: DropdownButtonHideUnderline(
                    child: DropdownButtonComponent<T>(
                      items: items,
...
class _DropdownButtonFormFieldState<T> extends FormFieldState<T> {
  @override
  DropdownFormFieldComponent<T> get widget =>
      super.widget as DropdownFormFieldComponent<T>;
  ...
  @override
  void didUpdateWidget(DropdownFormFieldComponent<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
